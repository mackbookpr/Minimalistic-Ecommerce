            ...
            <div className="grid lg:grid-cols-4 xl:grid-cols-5 md:grid-cols-3 sm:grid-cols-2 gap-2 mt-5">
                {Object.keys(imagesElectronics).map((imageName, index) => (
                    <Link key={index} to={`/Electronics/${getProductIdFromImageName('Electronics', imageName)}`}>
                        <div className="cursor-pointer relative border-gray-500 hover:border-black transition-all duration-400 border-2 h-[280px]">
                            <img src={imagesElectronics[imageName]} alt={imageName} className='object-cover h-[220px] w-[275px]' />
                ...
                {Object.keys(imagesSkinCare).map((imageName, index) => (
                    <Link key={index} to={`/SkinCare/${getProductIdFromImageName('Skincare', imageName)}`}>
                        <div className="relative cursor-pointer border-2 border-gray-500 hover:border-black h-[280px] transition-all duration-400">
                            <img key={index} src={imagesSkinCare[imageName]} alt={imageName} className='object-cover h-[220px] w-[275px]' />
                ...
                {Object.keys(imagesFurniture).map((imageName, index) => (
                    <div className="relative cursor-pointer border-2 border-gray-500 hover:border-black transition-all duration-400 h-[280px]">
                        <img key={index} src={imagesFurniture[imageName]} alt={imageName} className='object-cover w-[275px] h-[220px]' />
                        <div className="absolute bottom-0 w-full h-[50px]">Price</div>
                    </div>
                ))}
                {Object.keys(imagesKitchen).map((imageName, index) => (
                    <div className="relative cursor-pointer border-2 border-gray-500 hover:border-black transition-all duration-400 h-[280px]">
                        <img key={index} src={imagesKitchen[imageName]} alt={imageName} className='object-cover w-[275px] h-[220px]' />
                        <div className="absolute bottom-0 w-full h-[50px]">Price</div>
                    </div>
                ))}
            </div></div >
